...
from market.models import InvestmentRecord, Transaction
...
class ProfileView(LoginRequiredMixin, CountNewsMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        qs = InvestmentRecord.objects.filter(user=self.get_object())
        if qs.count() >= 1:
            context['net_worth'] = InvestmentRecord.objects.calculate_net_worth(self.get_object())
            context['investments'] = qs
            context['transactions'] = Transaction.objects.get_by_user(user=self.get_object())
        return context
